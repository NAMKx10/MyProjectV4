<?php
// نحصل على كائن الـ URI الحالي للوصول إلى أجزاء الرابط
$uri = service('uri');
// نحصل على الجزء الأول من الرابط (e.g., 'lookups', 'users', 'dashboard')
$segment1 = $uri->getSegment(1, 'dashboard');

// نعرّف مجموعات الصفحات لتحديد القائمة النشطة
...
$admin_pages       = ['users', 'roles', 'lookups', 'archive', 'settings'];

// نتحقق إذا كانت الصفحة الحالية تنتمي لأي من المجموعات
...
<ul class="navbar-nav">
  ...
  <li class="nav-item dropdown <?= $is_admin_active ? 'active' : '' ?>">
    ...
    <div class="dropdown-menu">
      ...
      <a class="dropdown-item <?= ($segment1 === 'roles') ? 'active' : '' ?>" href="<?= site_url('roles') ?>">الأدوار والصلاحيات</a>
